...
!
 subroutine raddriv11(flxu, flxd, hrs, hrl, hrsc, hrlc, &
  ...
  real, intent(in), dimension(ilg,lay,nxloc) :: clw_sub !< Subgrid-scale liquid water content \f$[gram/m^3]\f$
  real, intent(in), dimension(ilg,lay,nxloc) :: cic_sub !< Subgrid-scale ice water content \f$[gram/m^3]\f$
  ...
  real, parameter :: specirr =  1360.6306 !< total solar energy calculated by model
      ...
      do ib = 1, nbs
        ...
        !
          ...
          do isample = 1, nsample_sw(ib,ig,igh)
            ...
            ! currently iseedrow(:) = 0.0 so this generates the same inital number each time

            call kissvec(iseedrow, ran_num, il1, il2, ilg)
!print*, iseedrow, ran_num
            do i = il1, il2
...
!print*,rel(1,36), lwc(1,36), rei(1,36), iwc(1,36)
       ...
       !
            ...
            if (.not. gh) then ! perform integration over these ig values
              ...
              if (ib == 1) then
                ...
                !
                call raylev3(taur, ig, dpg, rmu3g, il1g, il2g, ilg, lay)
                !
              end if
              !
              ...
              call gasopts6(taug, gw, dpg, ib, ig, o3g, qg, co2g, ch4g, an2og, o2g, &
                            inptg, inpr, mcont, gci, omci, dt, rmu3g, il1g, il2g, ilg, lay)!!!
              !
	...
	!print COD for BAND 1
	if (ib == 1) then
	   do i =il1g,il2g
	      !print*,sum(taucsg(1,:))
	      cod_vis(i) = sum(taucsg(i,:)) 
	   enddo
	endif
        if (ib == 1) then
          do i = il1g, il2g
